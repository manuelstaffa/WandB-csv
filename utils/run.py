...
def save_upsampled(rgb_arrays, k=4, l=4):
    ...
    augs = [
        np.repeat(np.repeat(rgb, k, axis=0), l, axis=1)[:, :, [2, 1, 0]]
...
def main():
    ...
    parser.add_argument(
        ...
        "--dopamine_pooling",
        action="store_true",
        help="Use dopamine-like frameskipping",
    )
    parser.add_argument(
        "-m",
        "--modifs",
    ...
    parser.add_argument(
        "-a", "--agent", type=str, default="", help="Path to trained agent."
    )
    parser.add_argument(
        "-r",
        "--render",
    ...
    if args.human:
        ...
        while not done:
            ...
            action = (
                policy(torch.Tensor(obs).unsqueeze(0))[0]
                if args.agent
            ...
            child = next((o for o in objects if o.category == "Child"), None)

            if player and child:
                # Manhattan distance between player and child
                ep_distance = abs(player.x - child.x) + abs(player.y - child.y)
                print(
